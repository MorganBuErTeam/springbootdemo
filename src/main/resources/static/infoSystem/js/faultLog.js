...
$(function() {
	...
	$(".daochu").click(function() {
		...
		window.location = "../excel/errorLog?type=" + type +
...
//					var html = '<span class="agv_table_input CabinetPosition">' + (index + 1) + '</span>'
			...
			},
			{
				...
				cellStyle: formatTableT,
				formatter: function(value, row, index) {
					var html = '<input disabled ="disabled" type="text" class="agv_table_input"  value="' + (row.errorType ? row.errorType : "无") + '"></input>'
					return html;
			...
			},
			{
				...
				formatter: function(value, row, index) {
					...
					// if(row.agvCode == 1){
					// 	str="堆垛机";
					// }else if(row.agvCode==2){
					// 	str="输送链";
					// }else if(row.agvCode==3){
					// 	str="RFID";
					// }
					var html = '<input disabled ="disabled" type="text" class="agv_table_input taskInfo"  value="' + (row.agvCode ? row.agvCode : "无") + '"></input>'
					return html;
			...
			
			{
				...
				cellStyle: formatTableT,
				formatter: function(value, row, index) {
					var html = '<input disabled ="disabled" type="text" class="agv_table_input"  value="' + (row.sitCode ? row.sitCode : "无") + '"></input>'
					return html;
				}
			},
			{
				...
				cellStyle: formatTableT,
				formatter: function(value, row, index) {
					var html = '<input disabled ="disabled" type="text" class="agv_table_input remark"  value="' + (row.remark ? row.remark : "无") + '"></input>'
					return html;
				}
			},
